(function () {
  ...
  function isSidebarArea(element) {
    // 检测元素是否在侧边栏区域（通过位置和特征判断）
    if (!element) return false;
    var rect = element.getBoundingClientRect();
    // 检查元素是否在 header 内，如果在 header 内就不是侧边栏
    var header = qs('header') || qs('[class*="header"]');
    if (header && header.contains(element)) return false;
    // 侧边栏通常在左侧（viewport 左侧 25% 以内）且宽度较小
    if (rect.left < window.innerWidth * 0.2 && rect.width < window.innerWidth * 0.3) return true;
    if (rect.left < window.innerWidth * 0.1) return true;
    return false;
  }
  ...
  function mountInTopNav() {
    try {
      ...
      // 顶部导航栏：优先插入到语言选择器左边，否则 append 到末尾
      if (hdr && isBotAdminHeader(hdr)) {
        var langTarget = langNode && hdr.contains(langNode) ? langNode : null;
        if (langTarget) {
          hdr.insertBefore(btn, langTarget);
        } else {
          hdr.appendChild(btn);
        }
        btn.style.position = '';
      ...
      // 如果有 header 且语言节点在 header 内，也 append 到 header
      ...
      // 通用逻辑：最高优先级 - 语言切换按钮左侧
      if (langNode) {
        var parent = langNode.parentElement;
        if (parent && isVisible(parent)) {
          // 检测是否在侧边栏区域，如果在则将按钮固定到右上角
          if (isSidebarArea(parent)) {
          ...
          group = parent;
          var target = directChildWithin(group, langNode);
          btn.style.position = '';
          btn.style.top = '';
          btn.style.left = '';
          btn.style.right = '';
      ...
      // 插入到语言按钮的前面（左边）
      if (langNode) {
        var langTarget = directChildWithin(group, langNode);
        if (langTarget) group.insertBefore(btn, langTarget);
        else group.insertBefore(btn, group.firstElementChild || null);
      } else {
