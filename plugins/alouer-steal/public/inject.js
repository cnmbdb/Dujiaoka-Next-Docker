(function () {
  ...
  function syncThemeToIframe(theme) {
    ...
    try {
      iframe.contentWindow.postMessage({ type: 'ALR_THEME', theme: theme || detectTheme() }, '*');
    } catch (e) {}
  }

  function applyEmbedTheme(force) {
    var theme = detectTheme();
    if (!force && theme === ALR.lastTheme) return;
    ALR.lastTheme = theme;
  ...
  function showIframe(slug) {
    ...
    if (oldIframe) {
      var nextSrc = '/alouer-steal/alouer/' + slug + '/?embed=1&theme=' + detectTheme();
      if (oldIframe.getAttribute('src') !== nextSrc) {
        oldIframe.src = nextSrc;
      }
    ...
    wrapper.style.cssText = 'position:fixed;left:' + Math.max(0, mainRect.left) + 'px;top:' + Math.max(0, mainRect.top) +
      'px;right:0;bottom:0;display:flex;flex-direction:column;z-index:40;background:' +
  ...
  function restoreMain() {
    ALR.activeSlug = null;
    var wrapper = document.getElementById('alr-embed-wrapper');
  ...
  function mount() {
    ...
    if (!ALR.themeObserver) {
      ALR.themeObserver = new MutationObserver(function () {
        if (ALR.activeSlug) applyEmbedTheme();
      });
      ALR.themeObserver.observe(document.documentElement, { attributes: true, attributeFilter: ['class'] });
    }
  ...
  function ensureEmbedAlive() {
    ...
    var mainEl = getMainEl();
    if (!mainEl || !document.body.contains(mainEl)) return;
    var wrapper = document.getElementById('alr-embed-wrapper');
    if (wrapper) {
      var mainRect = mainEl.getBoundingClientRect();
      wrapper.style.left = Math.max(0, mainRect.left) + 'px';
      wrapper.style.top = Math.max(0, mainRect.top) + 'px';
    }
    var iframe = document.getElementById('alr-iframe');
    if (iframe) return;
