(function () {
  ...
  var panelUrl = cfg.panelUrl || (window.location.origin + '/appstore-expand/panel');
  var healthUrl = cfg.healthUrl || (window.location.origin + '/appstore-expand/health');
  var panelOrigin = '';
  try {
    panelOrigin = new URL(panelUrl, window.location.href).origin;
  } catch (_ignored) {
    panelOrigin = '';
  }
  var healthState = {
  ...
  function createBtn() {
    ...
    function togglePanel(evt) {
      ...
      iframeUrl.searchParams.set('dj_origin', window.location.origin);
      iframeUrl.searchParams.set('dj_domain', window.location.hostname);
